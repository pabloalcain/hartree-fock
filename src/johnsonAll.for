...
************************************************************************
      ...
      call output(gi,*901)
      stop
 901  write(IW6,*) ' Error exit in HF Program'
      stop
      end
...
***   read first data card
      ...
      write(6,*) 'Symbol A4'
      read(5,501) idn
      write(6,*) 'number of shells 1s 2s 2p 3s 3p..'
      read(5,*) jx
      write(6,*) 'nuclear charge'
      read(5,*) jz
      write(6,*) 'no valence electrons'
 501  format(A4)
      
C     read(5,1010) idn,jx,jz,jc !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c     IDN='  Ar'    ! element
c     JX=5          ! number of shells 1s 2s 2p 3s 3p
c     JZ=18         ! nuclear Chareg
      JC=0                      ! no valence electrons
      IW6=8
      OPEN(UNIT=IW6,FILE=IDN//'.out')
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      
      
      write(IW6,1000) idn
 1000 format('   Input data for ',a/)
      write(IW6,1010) idn,jx,jz,jc
 1010 format(a,3i4)
      if(jx.gt.NOR) then
         write(IW6,1011) ' Too many input orbitals !!  shell dim = ',NOR
 1011    format(a,i4)
         go to 901
      endif
      if(jc.eq.0) jc = jx
...
C     read(5,1020) ns(ja),ls(ja),js(ja) !!!!!!!!!!!!!!!!!!!!!!!!!!
         IF(JA.EQ.1) THEN
            NS(1)=1
            LS(1)=0
            JS(1)=0
...
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 1020    format(3i4)
         ...
         write(IW6,1030) lab(ja),js(ja),wi(ja)
...
C     H=0.02D0
      RMAX=30.D0
      H=1.D0/DFLOAT(NGP)*DLOG(RMAX/R0)
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
*     V(r) = h1*Vold(r) + (1.0-h1)*Vnew(r)
         ...
         do 210 i = 1,NGP
            zold(i) = z(i)
            zn = jz + Z_MADELUNG(R(I)) - (1.0d0-exc)*y(i)*r(i)
...
*     
 ...
 220        continue
            ...
            write(IW6,1040) lab(ja),wi(ja),wf(ja),ks(ja)
 1040       format('  E(',a,') =',2f16.5,6x,'k =',i3)
            wi(ja) = wf(ja) + dw
 250     continue

         write(IW6,1045) it,del
...
***   choose orbital with largest error   ***
 ...
 400     continue
         ...
         call exchan(ja,del,ifn,*901)
         dw(ja) = wf(ja) - temp
         write(IW6,1020) lab(ja),wf(ja),emax,kk
 1020    format(2x,a,f16.8,4x,1pe10.1,4x,'iterate ',i4)
 500  continue 

***   too many iterations

      write(IW6,*) ' Error in pickup: too many iterations'
      return 1
...
***   final convergence test   ***
 ...
 550  k = k - 1
      ...
      do 600 ja = 1,jc
         ...
         write(IW6,1030) lab(ja),wf(ja),emax,k
...
***   v(r) = 2 [ Vdir(r)-U(r) ]*pa(r)
 ...
 200  continue
      ...
      do 250 i = 2,ma
         v(i) = 2.0d0*(vdir(i) + (z(i) - jz - Z_MADELUNG(R(I)))/
     |        r(i) )*pf(i,ja)
 250  continue
...
***   Wave function for corrected orbital  
 ...
 800  continue
      write(IW6,*) ' Error in fock:  norm loop fails to converge'
      return 1
...
************************************************************************
      implicit real*8(a-h,o-z)
      parameter(NOR=30,NGP=1000,NSCAL=10)
      character*4 idn,lab
      COMMON /MAIN1/ IW6
      common/radial/r(NGP),rp(NGP),rpor(NGP),h
      common/labdat/idn,lab(NOR)
...
***   print out orbital energies
 ...
 1010 format('  Shell','  #el',6x,'  Energy')
      ...
      do 150 ja = 1,jx
         write(IW6,1020) lab(ja),js(ja),wf(ja)
...
***   calculate and print scalar products
 ...
 225           continue
               ...
               if(jns.eq.1) then
                  write(IW6,1200) (lab(jov(ii,ic)),ii = 1,icnt(ic))
                  write(  6,1200) (lab(jov(ii,ic)),ii = 1,icnt(ic))
 1200             format(/11x,8('    ',a,'> '))
               endif
 230        continue
            write(IW6,1210) lab(j),(over(ii),ii=1,ins)
 ...
 500  continue
      write(IW6,1500)
 1500 format(/)
C     open(unit=1,file='fort.1',form='unformatted',status='unknown')
C     rewind 1

C     write(1) idn,jz,jx,jc
C     write(1) r,rp,rpor,h
      pause
...
***   set up direct potential and starting df functions
 ...
 130  continue
      ...
      do 140 i = 1,NGP
         z(i) = jz + Z_MADELUNG(R(I))- y(i)*r(i) + zex(i)
 140  continue
...
*     write(IW6,1010) lab(ja),lab(jv),over
...
*     write(IW6,1020) lab(jv),lab(jv),over
...
***   Loop for proper norm
 ...
 380        continue
            write(IW6,*) ' Error in valence: too many norm iterations'
            return 1
 390        continue

            err = abs(( wf(jv) - temp )/wf(jv))

            write(IW6,1030) lab(jv),wf(jv),err,it
 1030       format(2x,a,f16.8,4x,1pe10.1,4x,'iterate ',i4)

*     write(IW6,1030) it, wf(jv), err
*     1030       format(4x,'iteration',i3,4x,' wf(au) =',1pe14.6,4x,
*     &                'err =',e10.2)

            if(err.le.eps) go to 450
 400     continue
         write(IW6,*) ' Error in valence: too many exchange iterations'
         return 1
 450     continue
...
*     write(IW6,1000) lab(ja),lab(ja),anorm,wa
...
c----------------------------------------------------------------------
      FUNCTION Z_MADELUNG(R)
      IMPLICIT REAL*8(A-H,O-Z)  
      Z_MADELUNG=0.d0           ! -V_MADELUNG(1,1,1,R)*R
c     =!-V_MADELUNG(1,1,1,R)*R FOR HALOGEN
c     =!+V_MADELUNG(1,1,1,R)*R FOR CATIONS
      zread=Z_MADELUNG
      RETURN
      END
      
C======================================================================
C     Madelung potential at the (positive) ion as a function of R
c     this is a continious fitting to the actual onion potential
c     itype=1   Yukawa-type Madelung potential
c     =2 Gaussian-type Madelung potential
c     IHALOG = identify the halogen
c     Icat   = identify the cation
c----------------------------------------------------------------------
      FUNCTION V_MADELUNG(IHALOG,ICAT,ITYPE,R)
      IMPLICIT REAL*8(A-H,O-Z)  
      DIMENSION ARED(4,4)
      
      DATA ARED(1,1)/4.04/      !FLI  ! en amstrong Amst=1.8897a0
      DATA ARED(1,2)/4.62/      !FNA
      DATA ARED(1,3)/5.35/      !FK 
      DATA ARED(1,4)/5.64/      !FRB 
      
      DATA ARED(2,1)/5.13/      !CLI
      DATA ARED(2,2)/5.64/      !CLNA
      DATA ARED(2,3)/6.29/      !CLK 
      DATA ARED(2,4)/6.58/      !CLRB
      
      DATA ARED(3,1)/5.50/      !BRLI
      DATA ARED(3,2)/5.97/      !BRNA
      DATA ARED(3,3)/6.60/      !BRK 
      DATA ARED(3,4)/6.85/      !BRRB 
      
      DATA ARED(4,1)/6.00/      !ILI
      DATA ARED(4,2)/6.47/      !INA
      DATA ARED(4,3)/7.07/      !IK 
      DATA ARED(4,4)/7.34/      !IRB
      
      DATA FVOL/-1.747564D0/, FSURF/-1.68155D0/
      
      IF(R.LT.1.D-10) R=1.D-10
      
      A=ARED(IHALOG,ICAT)*1.8897D0 
      VMADELUNG0=FVOL/(A/2.D0)
      
      
C     YUKAWA TYPE    ----------------------------------------
      IF(ITYPE.EQ.1) THEN
         B=0.3D0*A
         V_MADELUNG=( DEXP(-R/B)*(1.D0 + (1.D0+VMADELUNG0*B)*(R/B) +
     |        (0.5D0 + VMADELUNG0*B)*(R/B)**2)-1.D0)/R
      ENDIF
      
C     GAUSSIAN TYPE  ------------------------------------------
      IF(ITYPE.EQ.2) THEN
         B=0.7D0*A
         V_MADELUNG=(DEXP(-(R/B)**2)*(1.D0 + VMADELUNG0*B*(R/B) + (R/B)**2)
     |        -1.D0)/R
      ENDIF
